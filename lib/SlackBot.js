...
var mixpanel = Mixpanel.init('c51484be4e530ccf53648d6c36e801f0');
var intercomClient = new Intercom.Client('fke30fr8', '026189f29bc42dd1610d4506c49c83a81854c93b');
function Slackbot(configuration) {
    ...
    slack_botkit.getAuthorizeURL = function(team_id, scopes, state) {

        //var scopes = slack_botkit.config.scopes;
        var url = 'https://slack.com/oauth/authorize' + '?client_id=' +
            slack_botkit.config.clientId + '&scope=' + scopes.join(',') + '&state=' + state;
    ...
    slack_botkit.createOauthEndpoints = function(webserver, clientUrl, fullScopes, callback) {
        ...
        webserver.get('/login', function(req, res) {
            var mixPanelId = req.query.mixpanel_id;
            console.log("Mixpanel ID: " + mixPanelId);
            var state = mixPanelId;
            res.redirect(slack_botkit.getAuthorizeURL(undefined, ['identity.basic', 'identity.email', 'identity.avatar', 'identity.team'], state));
        });
                    ...
                    if (err) {
                        ...
                        slack_botkit.findTeamById(identity.team_id, function(err, team) {
                            ...
                            var isMissingScope = function(scopes, fullScopes) {
                                console.log("_------ isMissingScope_------ ");
                                console.log(scopes);
                                console.log(fullScopes);
                                var isMissing = false;
                            ...
                            var redirect = function(auth) {
                                var teamName = auth.team ? auth.team.name : auth.team_name;
                                var teamId = auth.team ? auth.team.id : auth.team_id;
                                var slackUserId = auth.user ? auth.user.id : auth.user_id;
                                ...
                                var lastName = nameArr.length > 1 ? nameArr[1] : "";
                                ...
                                intercomClient.users.create({
                                    ...
                                    custom_attributes: {
                                        ...
                                        scopes: JSON.stringify(auth.scope),
                                        firstname: firstName,
                                        lastname: lastName,
                                        user: userName,
                                        team: teamName,
                                        version: 'Built-on-Slack 1.0',
                                        addToSlackSeen: 'true'
                                    }
                                }, function(r) {
                                    if (r.errors) {
                                ...
                                var target = slack_botkit.getAuthorizeURL(undefined, fullScopes, state);
                                var url = clientUrl + "/#/add-to-slack?target=" + encodeURI(target);// + "&auth=" + encodeURI(JSON.stringify(auth));
                                console.log("######### Add to slack URL : " + url);
                                console.log("Redirecting to: " + url);
                                res.redirect(url);
                                res.end;
                            }
                            isAddToSlack = !isMissingScope(auth.scope, fullScopes);
                            ...
                            if (!isMissingScope(scopes, fullScopes)) {
                                ...
                                if (isMissingScope(auth.scope, fullScopes)) {
